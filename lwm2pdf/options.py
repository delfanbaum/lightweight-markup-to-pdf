...
# options parsing
def get_lwm2pdf_options(supported_file_types: str):
    cli_options_parser = argparse.ArgumentParser(prog='lwm2pdf',
                        usage='%(prog)s [options]',
                        description=f'Converts {supported_file_types} into PDFs based on default or user stylesheet (css).')
    ...
    cli_options_parser.add_argument('-i', '--input', 
                                    ...
                                    type=str, 
                                    required=True,
                                    help=f'the file to convert ({supported_file_types})')
